...
export default {
  ...
  computed: {
    ...
    defaultImage () {
      const finalWidth = this.getWidthAdaptedToWidthOnScreen(this.defaultWidth, this.widthOnScreen)
    ...
    smartphoneSrcset () {
      return this.getSrcsetSizes('smartphone', this.widthOnScreenSmartphone).map(
        (finalWidth) => {
          const result = this.getImageUrlWithWidthAndHeight(finalWidth, this.getHeightFromWidth(finalWidth)) + ` ${finalWidth}w`
          return result
        }
      ).join(', ')
    },
